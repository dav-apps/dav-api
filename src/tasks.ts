...
import type { TaskDependencies } from "./appDependencies.js"
import { userWasActive, saveTableObjectInRedis } from "./utils.js"

const timezone = "Etc/UTC"

export function setupTasks(dependencies: TaskDependencies) {
	const tasks = createTasks(dependencies)
	const scheduledTasks = [
		nodeCron.schedule("0 1 * * *", tasks.updateRedisCaches, { timezone }),
		nodeCron.schedule("0 0 0 * * *", tasks.createUserSnapshots, { timezone }),
		nodeCron.schedule("0 0 0 * * *", tasks.createAppUserSnapshots, {
			timezone
		}),
		nodeCron.schedule("0 3 * * 0", tasks.deleteSessions, { timezone }),
		nodeCron.schedule("*/10 * * * *", tasks.sendNotifications, { timezone })
	]
	return () => scheduledTasks.forEach(task => task.stop())
}

// Jobs can be run directly without configuring VAPID or registering timers.
export function createTasks({ prisma, redis, webPush }: TaskDependencies) {
	...
	async function sendNotifications() {
		const notifications = await prisma.notification.findMany({
			...
			include: {
				user: {
					include: {
						sessions: { include: { webPushSubscriptions: true } }
					}
				}
			}
		})
